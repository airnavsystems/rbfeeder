...
DUMP1090_VERSION ?= 1.0.12

AIRNAV_TYPE ?= raspberry
BUILD_DATETIME ?= $(shell date +'%Y%m%d%H%M00')

CPPFLAGS += -I. -D_POSIX_C_SOURCE=200112L -DMODES_DUMP1090_VERSION=\"$(DUMP1090_VERSION)\" -DAIRNAV_VER_PRINT=\"$(DUMP1090_VERSION)\" -DMODES_DUMP1090_VARIANT=\"dump1090-rb\" -DBDTIME=\"$(BUILD_DATETIME)\" -DF_ARCH=\"$(AIRNAV_TYPE)\" -DLOAD_STATIC_WOF
